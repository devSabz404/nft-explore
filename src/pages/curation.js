...
function Curation(){
    ...
    let query = await axios.get(`${API_ENDPOINT}/artist/artist-details`);
    return query.data;
    ...
    async function onAccept(email){
        await axios.post(`${API_ENDPOINT}/artist/invite`,{email})
        .then((res)=>{
          setSuccess(true)
    ...
    return(
        ...
        <Card title={item.user_name} style={{ width: 800 }}>
          ...
          <Button type="primary" htmlType="submit" onClick={()=>onAccept(item.user_email)}>Accept</Button>
        </Card>)}
        </>
        )
